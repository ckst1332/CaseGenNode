...
export const generateFullModelXlsx = async (caseData) => {
  ...
  // Generate sophisticated single-sheet model with detailed financial statements
...
const generateComprehensiveModelSheet = async (worksheet, caseData) => {
  ...
  // =====================================
  // SECTION 3: DETAILED FINANCIAL STATEMENTS  
  // =====================================
  
  // Income Statement
  currentRow += 3;
  worksheet.mergeCells(`A${currentRow}:L${currentRow}`);
  const incomeStatementHeader = worksheet.getCell(`A${currentRow}`);
  incomeStatementHeader.value = 'INCOME STATEMENT (5-Year Projections)';
  incomeStatementHeader.style = getSectionHeaderStyle();
  currentRow += 2;
  
  if (answerKey?.income_statement) {
    currentRow = await generateStatementSection(worksheet, answerKey.income_statement, 'Income Statement', currentRow);
  }
  ...
  if (answerKey?.cash_flow_statement) {
    currentRow = await generateStatementSection(worksheet, answerKey.cash_flow_statement, 'Cash Flow Statement', currentRow);
  }
  ...
  if (answerKey?.dcf_valuation && answerKey?.final_metrics) {
    await generateDCFSection(worksheet, answerKey.dcf_valuation, answerKey.final_metrics, currentRow);
  }
