...
export const generateFullModelXlsx = async (caseData) => {
  ...
  workbook.title = `${caseData.name || caseData.company_name} - Financial Model`;
  ...
  await generateSummarySheet(summarySheet, caseData);
  await generateFinancialStatementSheet(incomeSheet, answerKey.income_statement, 'Income Statement', caseData.year_0_baseline?.income_statement);
  await generateFinancialStatementSheet(balanceSheet, answerKey.balance_sheet, 'Balance Sheet', caseData.year_0_baseline?.balance_sheet);
  await generateFinancialStatementSheet(cashFlowSheet, answerKey.cash_flow_statement, 'Cash Flow Statement', caseData.year_0_baseline?.cash_flow_statement);
  await generateDCFSheet(dcfSheet, answerKey.dcf_valuation, answerKey.final_metrics);
