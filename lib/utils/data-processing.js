...
/**
 * Generate a financial statement section in the comprehensive sheet
 */
const generateStatementSection = async (worksheet, statementData, statementType, startRow) => {
  ...
  // Determine fields based on statement type
  let fields = [];
  ...
  worksheet.getCell(currentRow, 1).value = 'Line Item';
  worksheet.getCell(currentRow, 1).style = getTableHeaderStyle();
  
  statementData.forEach((item, index) => {
    worksheet.getCell(currentRow, index + 2).value = `Year ${item.year}`;
    worksheet.getCell(currentRow, index + 2).style = getTableHeaderStyle();
  });
  currentRow++;
  ...
  fields.forEach(field => {
    ...
    statementData.forEach((item, index) => {
      ...
      worksheet.getCell(currentRow, index + 2).value = typeof value === 'number' ? formatCurrency(value) : value || '-';
      worksheet.getCell(currentRow, index + 2).style = getTableDataStyle();
    });
    currentRow++;
  });
