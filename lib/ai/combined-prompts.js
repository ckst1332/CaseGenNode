...
export const getCombinedCaseAndModelPrompt = (industry = "Technology (SaaS)") => {
  const prompt = `
    You are an institutional financial modeling expert creating a sophisticated ${industry} DCF case study. Generate a UNIQUE, VARIED business scenario with realistic financials.

    **COMPANY VARIATION REQUIREMENTS:**
    Create one of these archetypes (rotate randomly):
    1. High-growth, early-stage (ARR: $5-15M, growth focus)
    2. Scale-up phase (ARR: $15-50M, efficiency focus) 
    3. Mature growth (ARR: $50-200M, profitability focus)
    4. International expansion (multi-geography complexity)
    5. Vertical-specific SaaS (industry-focused solution)
    ...
    **CRITICAL: LINE ITEM COMPLEXITY REQUIREMENTS:**
    - COGS must include hosting, support, payment processing (10-25% of revenue)
    - Sales & Marketing must include CAC, lead generation, events (25-50% of revenue)  
    - R&D must include engineering, product development, QA (15-30% of revenue)
    - G&A must include admin, legal, finance, HR (8-20% of revenue)
    - Depreciation must reflect infrastructure and software investments
    - Working capital changes must reflect AR, deferred revenue dynamics
    - CapEx must include infrastructure, software, office buildouts
    
    NEVER generate zero values for core operating line items - this reduces model realism.

    **REALISTIC FINANCIAL TARGETS (VP/MD APPROVED):**
    - IRR: 18-28% (institutional venture returns)
    - Revenue CAGR: 30-45% early years declining to 15-25% (sustainable growth)
    - Gross margins: 75-88% improving gradually over time
    - Rule of 40: >40% by Y3, NPV should be 3-8x starting ARR
    - LTV/CAC: 4-7x improving, FCF margin: 15-25% by Y5
    - EV/Revenue: 3-12x based on growth/profitability profile
